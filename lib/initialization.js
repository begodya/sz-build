...
function programConfig() {
  ...
  program.command('create <name>')
    .description('create a new project powered by sz-build')
    ...
    .action(async (name, cmd) => {
      ...
      store.cmd = 'create';
      store.dirname = name;
      // 清空控制台，并输出当前 sz-build 版本
      clearConsole('cyan', `SZ-BUILD v${requiredPackageVersion}`);
      // 判断参数 是否避免版本检测
      if (cmd.noversion) {
        isQuickCreact(cmd);
      } else {
        // 检测 sz-build 版本（通过 npm 获取 latest 版本号）
  ...
  program.command('test <name>')
    .description('sz-build in travis-ci test')
...
module.exports = function initialization() {
  // 检测 node 版本
  checkNodeVersion(requiredNodeVersion, 'sz-build');
  // commander 配置
  programConfig();
};