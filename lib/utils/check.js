...
function checkPackageVersion(url) {
  return new Promise((resolve, reject) => {
    spinner.start('Checking SZ-BUILD version');
    request(url, function (error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        if (semver.lte(version, requiredVersion)) {
          ...
          spinner.stop();
          console.log(chalk.red(
            `You are using SZ-BUILD v${requiredVersion}, But the latest version is v${version}.\nPlease upgrade your SZ-BUILD version. \n\n>> npm update sz-build -g`
          ));
          process.exit(1);
        }
      } else {
